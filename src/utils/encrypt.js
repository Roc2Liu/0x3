 ...
 * 
 * ⚠️ 安全说明：
 * 本工具仅用于简单混淆，避免明文直接泄露，不提供高安全性保护。
 * 
 * 特点：
 * 1. 使用 XOR 加密和 Base64 编码进行简单混淆
 * 2. 密钥基于域名和时间戳生成，同一秒内导出的文件密钥相同
 * 3. 适用于本地备份和迁移，防止明文直接暴露
 * 4. 不适用于需要高安全性的场景（如敏感数据、金融信息等）
 * 
 * 警告：
 * - 此加密方式可以被逆向，请勿用于保护敏感信息
 * - 仅用于防止配置文件以明文形式直接泄露
 * - 如需高安全性，请使用专业的加密库
 */
